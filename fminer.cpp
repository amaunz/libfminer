...
FMiner::FMiner() : chisq(0.95, &database), minfreq(2), type(2), do_backbone(false), updated(true), adjust_ub(true), do_pruning(true) {
  InitChisq();
}

//! Like standard constructor, but type and minimum frequency configurable
FMiner::FMiner(int type, unsigned int minfreq) : chisq(0.95, &database), minfreq(minfreq), type(type), do_backbone(false), updated(true), adjust_ub(true), do_pruning(true) {
...
FMiner::FMiner(int type, unsigned int minfreq, float chisq_val, bool do_backbone) : chisq(chisq_val, &database), minfreq(minfreq), type(type), do_backbone(do_backbone), updated(true), adjust_ub(true), do_pruning(true) {
